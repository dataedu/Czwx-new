...
public class SchIntroDescActivity extends BaseFragment {
	...
	protected void initialize(View view) {
		...
		if(DeviceUtil.checkNet()){//判断是否有网络，没网络到本地数据库获取
			...
			if (StringUtils.isNotEmpty(helper.getValue("xxjj"))){
				t.setText(helper.getValue("xxjj"));
				...
				try {
					if (result.getInt("code") == 200) {
						...
						if (StringUtils.isNotEmpty(result.getString("data"))) {
							...
							t.setVisibility(View.VISIBLE);
							helper.setValue("xxjj",result.getString("data").toString());
						}else {
